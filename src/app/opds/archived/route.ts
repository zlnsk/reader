...
export async function GET(req: NextRequest) {
  ...
  const { rows, total } = await fetchBooksPage(auth.email, { page, per, archived: true });
  const feed: Feed = {
    id: `${base}/opds/archived?page=${page}`,
    title: "Reader — Archived",
    updated: new Date().toISOString(),
    totalResults: total,
    itemsPerPage: per,
    startIndex: (page - 1) * per + 1,
    links: [
      ...paginationLinks(base, "/opds/archived", page, per, total),
