...
export async function GET(req: NextRequest) {
  ...
  const qstr = (url.searchParams.get("q") || url.searchParams.get("query") || "").trim();
  ...
  const { rows, total } = await fetchBooksPage(auth.email, { page, per, archived: "all", query: qstr });
  const feed: Feed = {
    id: `${base}/opds/search?q=${encodeURIComponent(qstr)}&page=${page}`,
    title: `Reader — Search: ${qstr}`,
    updated: new Date().toISOString(),
    totalResults: total,
    itemsPerPage: per,
    startIndex: (page - 1) * per + 1,
    links: [
      ...paginationLinks(base, `/opds/search?q=${encodeURIComponent(qstr)}`, page, per, total),
